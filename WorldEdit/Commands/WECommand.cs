...
namespace WorldEdit.Commands
{
	public abstract class WECommand
	{
		...
		public void SetTile(int i, int j, int tileType)
		{
			...
			switch (tileType)
			{
				case -1:
					...
					return;
				case -2:
					if (plr.HasPermission("worldedit.usage.liquids.lava"))
					{
						tile.active(false);
						tile.liquidType(1);
						tile.liquid = 255;
					...
					return;
				case -3:
					if (plr.HasPermission("worldedit.usage.liquids.honey"))
					{
						tile.active(false);
						tile.liquidType(2);
						tile.liquid = 255;
					...
					return;
				case -4:
					if (plr.HasPermission("worldedit.usage.liquids.shimmer"))
                    {
						tile.active(false);
						tile.liquidType(3);
						tile.liquid = 255;
					...
					return;
				case -5:
					if (plr.HasPermission("worldedit.usage.liquids.water"))
					{
						tile.active(false);
						tile.liquidType(0);
						tile.liquid = 255;
		...
		public bool TileSolid(int x, int y)
			=> x < 0 || y < 0 || x >= Main.maxTilesX || y >= Main.maxTilesY || (Main.tile[x, y].active() && Main.tileSolid[Main.tile[x, y].type]);

        public bool CanUseCommand() 
			=> CanUseCommand(x, y, x2, y2);

        public bool CanUseCommand(int x, int y, int x2, int y2)
        {
			CanEditEventArgs args = new(plr, x, y, x2, y2);
